...
def get_data_frontend_new():
    ...
    df_totals = df_table_launch_summary.groupby('configuration_name').agg({'id': 'count'}).reset_index()
    df_totals = df_totals.rename(columns={'id': 'launch_attempt_total'})
    df_totals[['launch_success_total', 'landing_attempt_total', 'landing_success_total']] = 0
    ...
    for index, row in df_table_launch_summary.iterrows():
        ...
        if row.status_abbrev != 'Failure':
            mask = df_totals['configuration_name'] == row.configuration_name
            df_totals.loc[mask, 'launch_success_total'] += 1

        for landing_attempt in row.landing_attempt:
            if landing_attempt == True:
                mask = df_totals['configuration_name'] == row.configuration_name
                df_totals.loc[mask, 'landing_attempt_total'] += 1

        for landing_success in row.landing_success:
            if landing_success == True:
                mask = df_totals['configuration_name'] == row.configuration_name
                df_totals.loc[mask, 'landing_success_total'] += 1
    ...
    df_cy = df_table_launch[df_table_launch['year'] == CURRENT_YEAR][selected_columns_summary]
    last_date = datetime.now() - relativedelta(years=1)
    df_cy_yoy = df_table_launch[(df_table_launch['year'] == CURRENT_YEAR - 1) & (df_table_launch['net'] <= last_date)][selected_columns_summary]

    df_py = df_table_launch[df_table_launch['year'] == CURRENT_YEAR - 1][selected_columns_summary]
    df_py_yoy = df_table_launch[df_table_launch['year'] == CURRENT_YEAR - 2][selected_columns_summary]

    df_ppy = df_table_launch[df_table_launch['year'] == CURRENT_YEAR - 2][selected_columns_summary]
    df_ppy_yoy = df_table_launch[df_table_launch['year'] == CURRENT_YEAR - 3][selected_columns_summary]

    # calculate the numbers
    df_cy = df_cy.groupby('configuration_name').agg({'id': 'count'}).reset_index()
    df_cy = df_cy.rename(columns={'id': 'launch_attempt_cy'})

    df_cy_yoy = df_cy_yoy.groupby('configuration_name').agg({'id': 'count'}).reset_index()
    df_cy_yoy = df_cy_yoy.rename(columns={'id': 'launch_attempt_cy_yoy'})

    df_py = df_py.groupby('configuration_name').agg({'id': 'count'}).reset_index()
    df_py = df_py.rename(columns={'id': 'launch_attempt_py'})

    df_py_yoy = df_py_yoy.groupby('configuration_name').agg({'id': 'count'}).reset_index()
    df_py_yoy = df_py_yoy.rename(columns={'id': 'launch_attempt_py_yoy'})

    df_ppy = df_ppy.groupby('configuration_name').agg({'id': 'count'}).reset_index()
    df_ppy = df_ppy.rename(columns={'id': 'launch_attempt_ppy'})

    df_ppy_yoy = df_ppy_yoy.groupby('configuration_name').agg({'id': 'count'}).reset_index()
    df_ppy_yoy = df_ppy_yoy.rename(columns={'id': 'launch_attempt_ppy_yoy'})

    # join all dataframes
    df_totals = pd.merge(df_totals, df_cy, on='configuration_name', how='left').fillna(0)
    df_totals = pd.merge(df_totals, df_cy_yoy, on='configuration_name', how='left').fillna(0)
    df_totals = pd.merge(df_totals, df_py, on='configuration_name', how='left').fillna(0)
    df_totals = pd.merge(df_totals, df_py_yoy, on='configuration_name', how='left').fillna(0)
    df_totals = pd.merge(df_totals, df_ppy, on='configuration_name', how='left').fillna(0)
    df_totals = pd.merge(df_totals, df_ppy_yoy, on='configuration_name', how='left').fillna(0)

    numeric_cols = df_totals.select_dtypes(include=['float64', 'int64']).columns
    df_totals[numeric_cols] = df_totals[numeric_cols].astype(int)

    # Save DataFrame to SQL table
    df_totals.to_sql(
        name='homepage_totals', 
        schema='spacexnumbers_frontend',  # Specify the schema here
        con=engine, 
        if_exists='replace',  # 'replace' will drop the table if it exists and create a new one. Use 'append' to add to an existing table.
        index=False  # If your DataFrame index is not needed in the SQL table
